...
def preprocess_find_best_frame(source_image):
    last_gen_frame = imageio.imread(source_image)
    last_gen_frame = resize(last_gen_frame, (256, 256))[..., :3]
    return (last_gen_frame)



def make_animation(source_image, driving_video, generator, kp_detector, he_estimator, relative=True, adapt_movement_scale=True, estimate_jacobian=True, cpu=False, free_view=False, yaw=0, pitch=0, roll=0, new_method = 1, freq = 6):
    SR_model = create_ESRGAN_model()
    with torch.no_grad():
        predictions = []

        last_image = imageio.imread("last_gen_frame.png")

        if new_method != 1:
            # Grapping a random frame from driving video as source image
            driving = torch.tensor(np.array(driving_video)[np.newaxis].astype(np.float32)).permute(0, 4, 1, 2, 3)

            idx = np.random.randint(0,driving.shape[2])
        ...
        for frame_idx in tqdm(range(driving.shape[2])):
            ...
            if new_method == 1:
                if frame_idx == 0 or frame_idx % freq == 0:
                    #source = driving_frame
                    #driving_sequence = driving[:, :, frame_idx:frame_idx+freq].cuda()
                    source_frame_idx = find_best_frame(last_image, driving_video, frame_idx, freq, cpu=opt.cpu)
                    print(f"new_source_frame_ID = {source_frame_idx}")
                    source = driving[:, :, source_frame_idx].cuda()
                    #performing down and then upsampling again
                    source = source.squeeze().permute(1,2,0) #make right shape
                    source = source.cpu()
                    source = torch.round(source*255).type(torch.LongTensor)
                    #print("first")
                    #print(torch.max(source))
                    source_lr = downscale_image(source, down_factor = 4) #downsampling factor 4
                    #print("second")
                    #print(np.max(source_lr))
                    source = SR_model(source_lr) #upsampling factor 4
                    #perform back to torch again
                    #print("third")
                    source = tf.cast(tf.clip_by_value(source, 0, 255),tf.uint8)
                    #print(np.max(source))
                    source = source.numpy() #first to numpy 
                    #print(np.max(source))
                    source = torch.Tensor(source).type(torch.LongTensor) #then to torch (pytorch)
                    source = source/255
                    source = source.permute(0,3,1,2) #then reshaping
                    #modifying image not to be black:
                    if not cpu:
                        source = source.cuda()
                    kp_canonical = kp_detector(source)
                    he_source = he_estimator(source)
                    kp_source = keypoint_transformation(kp_canonical, he_source, estimate_jacobian)
                    he_driving_initial = he_estimator(driving_frame)
                    #kp_canonical = kp_detector(driving_frame)
                    kp_driving_initial = keypoint_transformation(kp_canonical, he_driving_initial, estimate_jacobian)
            ...
            last_gen_frame = np.transpose(out['prediction'].data.cpu().numpy(), [0, 2, 3, 1])[0]
            
            predictions.append(last_gen_frame)

            last_image = img_as_ubyte(last_gen_frame)
            last_image = resize(last_image, (256, 256))[..., :3]
    return predictions

def find_best_frame(source, driving, index_start, freq, cpu=False):
    ...
    norm  = float('inf')
    frame_num = index_start
    for i, image in tqdm(enumerate(np.array(driving)[index_start:index_start + freq])):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--new_method", dest="new_method", type=int, default=None, help="Wheter our new method is applied or original")
    parser.add_argument("--freq", dest="freq", type=int, default=None, help="The frequency to send source images")
    ...
    if opt.find_best_frame or opt.best_frame is not None:
        ...
        predictions_forward = make_animation(source_image, driving_forward, generator, kp_detector, he_estimator, relative=opt.relative, adapt_movement_scale=opt.adapt_scale, estimate_jacobian=estimate_jacobian, cpu=opt.cpu, free_view=opt.free_view, yaw=opt.yaw, pitch=opt.pitch, roll=opt.roll, new_method = opt.new_method, freq = opt.freq)
        predictions_backward = make_animation(source_image, driving_backward, generator, kp_detector, he_estimator, relative=opt.relative, adapt_movement_scale=opt.adapt_scale, estimate_jacobian=estimate_jacobian, cpu=opt.cpu, free_view=opt.free_view, yaw=opt.yaw, pitch=opt.pitch, roll=opt.roll, new_method = opt.new_method, freq = opt.freq)
