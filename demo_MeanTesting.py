        ...
        for frame_idx in tqdm(range(driving.shape[2])):
            ...
            if new_method == 1:
                if frame_idx == 0 or frame_idx % freq == 0:
                    ...
                    if frame_idx > 0:
                        #Previous calculated
                        yaw, pitch, roll, t, exp = he_source['yaw'], he_source['pitch'], he_source['roll'],he_source['t'], he_source['exp']
                        # new calculated
                        he_source = he_estimator(source)
                        #generate mean
                        #print("first")
                        #print(he_source["t"].shape)
                        #print("second3")
                        #print(t.squeeze().unsqueeze(0).shape)
                        #he_source_next = he_source.copy()
                        #he_source_next["t"] = (he_source["t"]+t.squeeze().unsqueeze(0)) / 2
                        #he_source_next["exp"] =  (he_source["exp"]+exp) / 2
                        
                        ##########################################
                        #kp_val = kp_canonical["value"]
                        #kp_canonical = kp_detector(source)
                        #kp_canonical["value"] += kp_val/2 
                    else:
                        he_source_next = he_estimator(source)
                        he_source = he_estimator(source)
                        #kp_canonical = kp_detector(source)
                        source_for_gen = source
                    kp_source = keypoint_transformation(kp_canonical, he_source, estimate_jacobian)
