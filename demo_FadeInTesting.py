...
def fadeIn (img1, img2): #pass images here to fade between
  ...
  for IN in range(1,5):
    fadein = IN/5.0
    dst = cv.addWeighted(img1, fadein, img2, (1-fadein), 0.0)#linear $
    frames.append(dst)
  return frames
        ...
        for frame_idx in tqdm(range(driving.shape[2])):
            ...
            if new_method == 1:
                if frame_idx == 0 or frame_idx % freq == 0:
                    ...
                    if frame_idx != 0:
                        source = driving[:, :, frame_idx-1].cuda()
                    else:
                        source = driving[:, :, frame_idx-1].cuda()
...
if __name__ == "__main__":
    ...
    for frames_idx in range(1, len(predictions)):
        if frames_idx%opt.freq == 0:
            new_predictions = new_predictions + fadeIn(predictions[frames_idx+1], predictions[frames_idx])
        else :
            new_predictions.append(predictions[frames_idx])

    imageio.mimsave(opt.result_video, [img_as_ubyte(frame) for frame in new_predictions], fps=fps)
